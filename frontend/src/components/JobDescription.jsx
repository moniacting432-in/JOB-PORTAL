...
const JobDescription = () => {
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        ...
        if (res.data.success) {
          ...
          setIsApplied(res.data.job.applications.some(application=>application.applicant == user?._id)) //Ensure the stae is in async with freshed data
        }
      } catch (error) {
        console.log(error);
      }
    ...
    fetchSingleJob ();
  }, [jobId,dispatch,user?._id]); // ✅ Fixed dependency array
  return (
        ...
        onClick = {isApplied ? null : applyJobHandler}
          ...
          className={` text-white rounded-lg ${
            isApplied
              ...
              : "bg-[#7209b7] font-white hover:bg-[#5f32ad]"
          }`}
        >
